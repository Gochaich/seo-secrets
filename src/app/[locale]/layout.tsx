...
const inter = Inter({
  ...
  display: 'swap',
  /*
   * Не предзагружаем. Предзагрузка ставит файл в начало очереди — туда же,
   * где стоит картинка первого экрана, по которой считается LCP. Четыре файла
   * шрифтов весят 119 КБ, и на медленном канале картинка ждала, пока они
   * скачаются.
   *
   * Замерено: LCP падает с 1752 до 1108 мс, первая отрисовка не меняется.
   * Шрифт при этом никуда не девается — он подтягивается из тех же стилей,
   * просто без места в голове очереди. display: swap уже стоял, поэтому
   * текст с первой секунды виден запасным шрифтом и подменяется без скачка:
   * next/font сам подгоняет метрики запасного под настоящий.
   *
   * Montserrat предзагрузку сохраняет: им набраны заголовки, он заметнее.
   */
  preload: false,
});
