...
const inter = Inter({
  ...
  display: 'swap',
  /*
   * Предзагрузку оставляем включённой. Пробовали выключить ради LCP —
   * по эмуляции на десктопе выходило лучше на 650 мс, но на боевом замере
   * PageSpeed стало хуже. Причина понятна задним числом: на мобильном
   * картинка первого экрана скрыта, и элементом LCP становится ТЕКСТ,
   * набранный этим самым Inter. Без предзагрузки он приезжает позже,
   * текст перерисовывается при подмене шрифта — и LCP уезжает вправо.
   *
   * Вывод на будущее: мерить надо мобильную версию, её и проверяет
   * PageSpeed, и первый экран там устроен иначе.
   */
});
